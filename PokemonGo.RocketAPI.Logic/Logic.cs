    ...
    public class Logic
    {
        ...
        private async Task DisplayPlayerLevelInTitle(bool updateOnly = false)
        {
            ...
            if (playerStat != null)
            {
                ...
                var message =
                    $"{playerName} | Лвл {playerStat.Level}: {playerStat.Experience - playerStat.PrevLevelXp - xpDifference}/{playerStat.NextLevelXp - playerStat.PrevLevelXp - xpDifference}XP";
        ...
        private async Task EvolveAllPokemonWithEnoughCandy(IEnumerable<PokemonId> filter = null)
        {
            ...
            foreach (var pokemon in pokemonToEvolve)
            {
                ...
                {
                    Logger.Write($"{pokemon.PokemonId} удачно за {evolvePokemonOutProto.ExpAwarded}xp",
                        LogLevel.Evolve);
                    await DisplayPlayerLevelInTitle(true);
                }
                else
                    Logger.Write(
                        $"Ошибка {pokemon.PokemonId}. EvolvePokemonOutProto.Result was {evolvePokemonOutProto.Result}, прекращаю эволюцию{pokemon.PokemonId}",
                        LogLevel.Evolve);
        ...
        public async Task Execute()
        {
            Git.CheckVersion();
            Logger.Write(
                $"Убедитесь в правильности координат! Lat: {_client.CurrentLat} Lng: {_client.CurrentLng}",
                LogLevel.Warning);
            Thread.Sleep(3000);
            Logger.Write($"Осуществляю вход: {_clientSettings.AuthType}");
            ...
            while (true)
            {
                ...
                {
                    Logger.Write("Время сессии закончилось!");
                }
        ...
        private async Task TransferDuplicatePokemon(bool keepPokemonsThatCanEvolve = false)
        {
            ...
            foreach (var duplicatePokemon in duplicatePokemons)
            {
                ...
                Logger.Write(
                    $"{duplicatePokemon.PokemonId} с {duplicatePokemon.Cp} ({CalculatePokemonPerfection(duplicatePokemon).ToString("0.00")} % ) CP (Лучший: {bestPokemonOfType.Cp} | ({CalculatePokemonPerfection(bestPokemonOfType).ToString("0.00")} % ))",
                    LogLevel.Transfer);
                await Task.Delay(500);
            }
        }
        ...
        public async Task UseBerry(ulong encounterId, string spawnPointId)
        {
            ...
            Logger.Write($"Использованно, осталось: {berry.Count}", LogLevel.Berry);
            await Task.Delay(3000);
        }

        private async Task DisplayHighests()
        {
            Logger.Write("====== Высший CP ======", LogLevel.Info, ConsoleColor.Yellow);
            var highestsPokemonCP = await _inventory.GetHighestsCP(20);
            foreach (var pokemon in highestsPokemonCP)
                Logger.Write(
                    $"# CP {pokemon.Cp}\t| ({CalculatePokemonPerfection(pokemon).ToString("0.00")}\t% perfect) Имя: '{pokemon.PokemonId}'",
                    LogLevel.Info, ConsoleColor.Yellow);
            Logger.Write("====== Процент лучшего ======", LogLevel.Info, ConsoleColor.Yellow);
