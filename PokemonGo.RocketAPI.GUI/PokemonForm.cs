    ...
    public partial class PokemonForm : Form
    {
        ...
        private async Task Execute()
        {
            ...
            foreach (var pokemon in pokemons)
            {
                ...
                listViewItem.ToolTipText = "Конфеты: " + currentCandy;
        ...
        {
            lbPokeListLoading.Text = "Загрузка...";
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Потверждение ?\n", "Потвердите", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                foreach (ListViewItem item in pokemonListView.SelectedItems)
                {
                    await _client.TransferPokemon((ulong)item.Tag);
                    Logger.Write($"Трансфер {item.SubItems[1].Text} с {item.SubItems[2].Text} CP и IV из {item.SubItems[3].Text}.");
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Вы действительно хотите эвовлнуть выбранных покемонов?", "Потдверждение", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                foreach (ListViewItem item in pokemonListView.SelectedItems)
                {
                    ...
                    if (newPokemon.Result == EvolvePokemonStatus.PokemonEvolvedSuccess)
                        MessageBox.Show($"Поздравляем вас с новым покемоном {newPokemon.EvolvedPokemon.PokemonType.ToString()} with {newPokemon.EvolvedPokemon.Cp} CP!");
                    else if (newPokemon.Result == EvolvePokemonStatus.FailedInsufficientResources)
                        MessageBox.Show("Недостаточно средств!");
                    else
                        MessageBox.Show($"Ошибка: {newPokemon.Result.ToString()}");
            ...
            if (dialogResult == DialogResult.Yes)
            {
                foreach (ListViewItem item in pokemonListView.SelectedItems)
                {
                    ...
                    if (poweredUpPokemon.Result == EvolvePokemonStatus.FailedInsufficientResources)
                        MessageBox.Show("Недостаточно пыли!");
                    else if (poweredUpPokemon.Result == EvolvePokemonStatus.FailedPokemonCannotEvolve)
                        MessageBox.Show("Слишком маленький уровень тренера");
                    else if (poweredUpPokemon.Result == EvolvePokemonStatus.PokemonEvolvedSuccess)
                        MessageBox.Show($"Успешный апгрейд! Cp: {poweredUpPokemon.EvolvedPokemon.Cp}");
                    else
                        MessageBox.Show($"Ошибка: {poweredUpPokemon.Result.ToString()}");
        ...
        private void pokemonListView_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                if (pokemonListView.FocusedItem.Bounds.Contains(e.Location) == true)
                {
                    ...
                    myContextMenuStrip.Items.Add("Трансфер выбранных покемонов", null, transferSelectedToolStripMenuItem_Click);
                    myContextMenuStrip.Items.Add("Эвовлв выбранных покемонов", null, evolveSelectedToolStripMenuItem_Click);
                    myContextMenuStrip.Items.Add("Апгрейд выбранных покемонов", null, powerupSelectedToolStripMenuItem_Click);
